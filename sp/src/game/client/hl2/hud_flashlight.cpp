...
void CHudFlashlight::Paint()
{
	...
	float flashBattery = pPlayer->m_HL2Local.m_flFlashBattery;
	float flashBatteryFrac = flashBattery / 100.0f;

	int barWidth = m_flBarWidth * flashBatteryFrac;

	Color clrFlashlight;
	clrFlashlight = flashBattery < 25.0f ? gHUD.m_clrCaution : gHUD.m_clrNormal;
	...
	if (bIsOn == false && flashBattery >= 100.0f)
		return;
	...
	surface()->DrawSetColor(clrFlashlight[0], clrFlashlight[1], clrFlashlight[2], 40);
