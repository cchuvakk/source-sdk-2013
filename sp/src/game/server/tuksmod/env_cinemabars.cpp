...
private:
	void BroadcastShow(bool bShow)
	{
		...
		int modeFlags = 0;
		if (m_bTop) modeFlags |= 1;
		if (m_bBottom) modeFlags |= 2;
		...
		if (gpGlobals->maxClients <= 1)
		{
			...
			if (pPlayer)
			{
				...
				UserMessageBegin(filter, "CinemaBars");
				WRITE_BYTE(bShow ? 1 : 0);
				if (bShow)
				{
					WRITE_SHORT(m_iHeight);
					WRITE_BYTE(modeFlags);
					WRITE_BYTE(m_color.r);
					WRITE_BYTE(m_color.g);
					WRITE_BYTE(m_color.b);
					WRITE_BYTE(m_alpha);
					WRITE_BYTE(m_bCoverHud ? 1 : 0);
					WRITE_FLOAT(m_flDuration);
					WRITE_FLOAT(m_flAnimIn);
					WRITE_FLOAT(m_flAnimOut);
				}
				MessageEnd();
			}
			return;
